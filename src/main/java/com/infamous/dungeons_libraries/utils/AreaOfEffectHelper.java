...
/*    public static boolean applyElementalEffectsToNearbyEnemies(PlayerEntity playerIn, int limit, float distance) {
            ...
            if (nearbyEnemies.size() >= i + 1) {
                ...
                switch (randomEffectId){
                    ...
                    case 2:
                        ...
                        break;
                    case 3:
                        nearbyEntity.setRemainingFireTicks(8);
                        break;
                }
            }
        }
    ...
    public static List<LivingEntity> getNearbyEnemies(LivingEntity origin, float distance, World world) {
        return world.getLoadedEntitiesOfClass(LivingEntity.class, origin.getBoundingBox().inflate(distance), (nearbyEntity) -> AbilityHelper.canApplyToEnemy(origin, nearbyEntity));
    }
